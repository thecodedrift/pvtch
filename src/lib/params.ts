...
export const paramsToObject = <T extends Record<string, any>>(
  ...
  Object.keys(defaults).forEach((key) => {
    ...
    if (handlers[key]) {
      result[key as keyof T] = handlers[key](paramValue) ?? defaults[key];
    } else {
      result[key as keyof T] = paramValue ?? defaults[key];
    }
  });
  return result;
};
