...
import { IRequest, json, RequestHandler, text } from "itty-router";
import {
  tokenDataKeyPrefix,
  twitchDataKeyPrefix,
  TwitchUserData,
...
type TwitchUserDataResponse = {
  data: {
    ...
    view_count: number;
    // email: "not-real@email.com"; (not here, didn't ask)
    created_at: string;
  }[];
};

export const authCallback: RequestHandler<IRequest, [Env]> = async (
  request,
  env
) => {
  const error = [request.query.error].flat()[0];
  const errorDescription = [request.query.error_description].flat()[0];
  const code = [request.query.code].flat()[0];

  // if there is an error, redirect to the pvtch errror page
  if (error) {
    const u = new URL(env.PVTCH_APP_URL || "http://localhost:3000/");
  ...
  const tokenData: TwithchTokenResponse = await tokenResponse.json();
  ...
  if (existing) {
    ...
    const redirectResponseWithToken = new Response(null, {
      status: 302,
      headers: {
        Location: env.PVTCH_APP_URL || "http://localhost:3000/",
...
their twitch data is stored in kv under their twitch user id (not login)
we generate a random token for them to use our API with
we then store both relationships in kv:
token -> twitch user id
twitch user id -> token

These are internal KV items. They are small enough we're not worried about a TTL.
*/
