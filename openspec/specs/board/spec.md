...
Streamer-owned, anonymous-by-URL markdown info card. Each board has a public read-only OBS overlay URL and a separate edit URL that mods can use without a login. Polling-based realtime; presence + optimistic concurrency prevent silent overwrites. History with revert. Per-user limits. Server-side markdown render is XSS-safe by configuration (no raw HTML, restricted link schemes, image syntax disabled).
